...
from src.pipeline.helpers.helpers import _extract_patient_id, _is_planning_ct_path
from src.pipeline.helpers.helpers_doses import (
    ...
    _select_dose_for_anatomy,
    _warp_planning_masks_for_predicted_dose,
...
def _clinical_metric_values(
    ...
    ------
    ValueError
        If the dose and mask shapes differ or a configured Dx volume
...
def _append_single_dose_rows(
    ...
    ------
    ValueError
        If a configured Dx volume percentage is outside ``[0, 100]``.
    """
...
def _evaluate_dose_on_real_structures(
    ...
    ----------
    dose_groups : dict[str, Path]
        Candidate or base dose paths keyed by patient identifier.

    reference_groups : dict[str, Path]
        Clinical fraction-1 dose paths keyed by patient identifier, used as
        fallbacks when a patient dose is unavailable.

    config : MaisiTestingConfig
        Configuration containing processed CT, structure, dose, and clinical
        metric settings.

    dose_source : str
        Source name used to identify selected patient-level doses in the
        returned rows.

    force_clinical : bool, optional
        If true, always select the clinical fraction-1 dose.

    Returns
    -------
    rows : list[dict[str, Any]]
        Long-form clinical metric records for valid real-anatomy scenarios.

    Raises
    ------
    ValueError
        If a selected dose is empty, non-finite, not three-dimensional, has an
        unsupported extension, or a configured Dx percentage is invalid.
    """
...
def evaluate_original_anatomy_comparison(config: MaisiTestingConfig) -> None:
    ...
    ------
    FileNotFoundError
        If the candidate or clinical dose directory does not exist.

    ValueError
        If no relevant candidate or clinical fraction-1 dose files are found,
        more than one relevant dose resolves to the same patient ID, a selected
        dose is invalid, or a configured Dx percentage is outside
        ``[0, 100]``.
    """
    ...
    if rows:
        ...
        logger.warning("Skipping original anatomy comparison: no comparable patient-level data were found")
...
def evaluate_scenario_robustness(
    ...
    ------
    FileNotFoundError
        If the clinical dose directory does not exist.

    ValueError
        If no clinical fraction-1 dose files are found, more than one relevant
        dose resolves to the same patient ID, a selected dose is invalid, or a
        configured Dx percentage is outside ``[0, 100]``; also if generated
        scenario rows omit columns required for the summary.
    """
...
def evaluate_base_dose_smoke_test(
    ...
    ValueError
        If no clinical fraction-1 dose files are found, more than one dose
        resolves to the same patient ID, a selected dose is invalid, or a
        configured Dx percentage is outside ``[0, 100]``.

    KeyError
        If generated smoke-test rows omit a column required for comparison.
...
def evaluate_predicted_doses(
    ...
    ------
    FileNotFoundError
        If the predicted or clinical dose directory does not exist.

    ValueError
        If no relevant predicted or clinical fraction-1 dose files are found,
        or if more than one relevant dose resolves to the same patient ID.
